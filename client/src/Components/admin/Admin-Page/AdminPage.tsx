...
function AdminPage(vacations: IVacation) {
    ...
    Swal.fire({
      ...
      try {
        if (result.isConfirmed) {
          axios.delete(`https://traveleasy-proj.herokuapp.com/vacations/${vacationId}`);
          dispatch({ type: ActionType.RemoveVacation, payload: { vacationId } });
          console.log("vacation removed");
          Swal.fire("Deleted!", "Vacation have removed.", "success");
  ...
  useEffect(() => {
    axios
      .get("https://traveleasy-proj.herokuapp.com/vacations/")
      .then((response) => {
            ...
            <div className="post-module" data-aos="fade-in">
              <div className="thumbnail">
                <AiOutlineClose
                  ...
                  onClick={() => onDeleteVacationClick(vacation.vacationId)}
                ></AiOutlineClose>
                <div className="date">
              ...
              <div className="post-content">
                <p className="title">
                  <ImLocation color="#8ec0d4" />
                  {vacation.location}{" "}
                </p>
                <p className="sub_title">
                  <GiAirplaneDeparture /> Depart:<b>{vacation.start_date}</b> <br /> <GiAirplaneArrival /> Return:{" "}
                  <b>{vacation.end_date}</b>{" "}
                </p>
